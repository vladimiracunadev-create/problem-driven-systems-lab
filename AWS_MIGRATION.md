...
> Plan operativo y honesto para mover el laboratorio (7 stacks operativos · 13 casos cada uno · 91 endpoints) desde Docker Compose local hacia AWS. Tres rutas alternativas, costos con rango realista, diagramas navegables y un mapping explícito de **cómo AWS cierra cada hallazgo del [SECURITY.md](SECURITY.md) sin tocar código del lab**.
...
- Estado real del repo a la fecha de este plan: **7 stacks operativos** (PHP, Python, Node, Java, .NET, Go, Rust), **91 endpoints** detrás de 7 hubs simétricos (`:8100`, `:8200`, `:8300`, `:8400`, `:8500`, `:8600`, `:8700`), portal en `:8080` y observabilidad PHP-only en `:9091` / `:3001`.
...
| `/php/*` | `tg-php-lab` | Dispatcher PHP (13 casos internos); casos 01/02 conectan a RDS pg-01/pg-02 |
| `/py/*` | `tg-python-lab` | Dispatcher Python con 13 casos internos |
| `/node/*` | `tg-node-lab` | Dispatcher Node con 13 casos internos |
| `/java/*` | `tg-java-lab` | Dispatcher Java con 13 casos internos |
| `/dotnet/*` | `tg-dotnet-lab` | Dispatcher .NET con 13 casos internos |
