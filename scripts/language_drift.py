...
def main() -> int:
    ...
    body = (
        ...
        "Al subir una version hay que tocar **tres** cosas, no una: el `Dockerfile` de los 12 "
        "casos del stack, la tabla de versiones del `README.md`, y el `comparison.md` de los "
        "casos cuya primitiva cambie. `scripts/check-language-versions.sh` verifica las dos "
        "primeras en cada PR; la tercera es criterio humano.\n\n"
