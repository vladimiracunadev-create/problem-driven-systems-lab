...
|- CHANGELOG.md
|- ROADMAP.md
|- compose.root.yml       ← PHP: 14 casos + portal + DB + observabilidad
|- compose.python.yml     ← Python: 14 casos, stdlib pura
...
Cada lenguaje operativo tiene su propio compose en la raíz — un comando levanta los 14 casos de ese lenguaje:

- `compose.root.yml` — PHP: portal (`8080`) + dispatcher PHP `php-lab` (`8100`, 14 casos internos en `:9001-:9014`) + PostgreSQL (casos 01–02) + worker case01 + Prometheus (`9091`) + Grafana (`3001`)
- `compose.python.yml` — Python: dispatcher único con 14 casos internos (`8200`), stdlib pura, sin dependencias externas
- `compose.nodejs.yml` — Node.js 22: dispatcher único con 14 casos internos (`8300`), stdlib pura (incluye `node:sqlite` built-in usado en caso 02), sin dependencias externas
- `compose.java.yml` — Java 21: dispatcher único con 14 casos internos (`8400`), JDK built-in (`HttpServer`, `HttpClient`), sin Maven
- `compose.dotnet.yml` — .NET 8: dispatcher único con 14 casos internos (`8500`), BCL built-in (`HttpListener`, `System.Text.Json`)
- `compose.go.yml` — Go 1.23: dispatcher único con 14 casos internos (`8600`), stdlib (`net/http`, `httputil.ReverseProxy`)
- `compose.rust.yml` — Rust 1.83: dispatcher único con 14 casos internos (`8700`); `std` no trae HTTP, la capa va sobre `TcpListener`
...
| Pieza | Rol |
| --- | --- |
| `compose.root.yml` | portal (`8080`) + `php-lab` dispatcher (`8100`, 14 casos PHP como subprocesos internos) + DB caso 01-02 + worker + Prometheus + Grafana |
| `compose.python.yml` | dispatcher Python (`8200`) con los 14 casos internos, stdlib pura, sin dependencias externas |
| `compose.nodejs.yml` | dispatcher Node.js (`8300`) con los 14 casos internos, stdlib pura |
| `compose.java.yml` | dispatcher Java (`8400`) con los 14 casos internos, JDK built-in (sin Maven) |
| `compose.dotnet.yml` | dispatcher .NET 8 (`8500`) con los 14 casos internos, BCL built-in |
| `compose.go.yml` | dispatcher Go 1.23 (`8600`) con los 14 casos internos, stdlib |
| `compose.rust.yml` | dispatcher Rust 1.83 (`8700`) con los 14 casos internos, capa HTTP propia |
...
**OPERATIVO** = lógica real, Docker funcional, evidencia observable. Los 14 casos × 7 stacks (PHP / Python / Node.js 22 / Java 21 / .NET 8 / Go 1.23 / Rust 1.83) son todos `OPERATIVO` — paridad funcional completa con primitivas idiomáticas distintas por runtime.
