...
| Patron | Uso |
| --- | --- |
| `compose.root.yml` | Portal + 19 casos PHP en una sola entrada |
| `compose.python.yml` | Hub Python con los 19 casos en un solo contenedor |
| `compose.nodejs.yml` | Hub Node.js con los 19 casos en un solo contenedor |
| `compose.java.yml` | Hub Java 21 con los 19 casos en un solo contenedor |
| `compose.dotnet.yml` | Hub .NET 8 con los 19 casos en un solo contenedor |
...
| --- | --- |
| `nginx-hub` (path routing) | `php-lab` (dispatcher hace el routing **y** ejecuta los 19 casos) |
...
- `compose.python.yml` y `compose.nodejs.yml` deben dejar visibles los 19 casos del stack respectivo desde `localhost:8200` y `localhost:8300`.
- `compose.java.yml` debe dejar visibles los 19 casos desde `localhost:8400`.
- `compose.dotnet.yml` debe dejar visibles los 19 casos desde `localhost:8500`.
- `compose.go.yml` debe dejar visibles los 19 casos desde `localhost:8600`.
- `compose.rust.yml` debe dejar visibles los 19 casos desde `localhost:8700`.
