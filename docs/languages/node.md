...
> **Versión fijada:** `22` (LTS) · **Imagen base:** `node:22-alpine` · **Hub:** `:8300` · **Casos operativos:** 12 / 12
...
Agregado de los veredictos de las 11 comparativas que rankean: **0 primeros puestos, media 4.5**.
...
- 🥉 **Tercero en 08** — `EventEmitter` + `Proxy` es lo más idiomático del laboratorio para un bus de eventos.
...
Los 12 casos quedan servidos en `http://localhost:8300/NN/`. Cada caso trae además su propio `compose.yml` para correrlo aislado — útil en los casos 01 y 11, donde la medición del event loop necesita el runtime sin ruido de los otros once casos.
