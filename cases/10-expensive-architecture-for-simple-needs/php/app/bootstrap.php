...
{
    $dir = sys_get_temp_dir() . '/pdsl-case10';
    if (!is_dir($dir)) {
        mkdir($dir, 0777, true);
    }
...
{
    return storageDir() . '/architecture-state.json';
}
...
function recordRequestTelemetry(string $uri, int $status, float $elapsedMs, ?array $context): void
{
    ...
    if ($context !== null) {
        $mode = (string) ($context['mode'] ?? 'complex');
        if (isset($telemetry['modes'][$mode])) {
            $modeBucket = &$telemetry['modes'][$mode];
            $scenario = (string) ($context['scenario'] ?? 'basic_crud');
            $modeBucket['by_scenario'][$scenario] = ($modeBucket['by_scenario'][$scenario] ?? 0) + 1;
            appendModeSample($modeBucket, 'monthly_cost_samples', (float) ($context['monthly_cost_usd'] ?? 0));
            appendModeSample($modeBucket, 'service_count_samples', (float) ($context['services_touched'] ?? 0));
            appendModeSample($modeBucket, 'lead_time_samples', (float) ($context['lead_time_days'] ?? 0));
            appendModeSample($modeBucket, 'coordination_samples', (float) ($context['coordination_points'] ?? 0));
...
function telemetrySummary(array $telemetry): array
{
    ...
    foreach (($telemetry['modes'] ?? []) as $mode => $bucket) {
        $costSamples = $bucket['monthly_cost_samples'] ?? [];
        $serviceSamples = $bucket['service_count_samples'] ?? [];
        $leadSamples = $bucket['lead_time_samples'] ?? [];
        $coordinationSamples = $bucket['coordination_samples'] ?? [];
        $modes[$mode] = [
            'successes' => (int) ($bucket['successes'] ?? 0),
            'failures' => (int) ($bucket['failures'] ?? 0),
            'avg_monthly_cost_usd' => count($costSamples) > 0 ? round(array_sum($costSamples) / count($costSamples), 2) : 0.0,
            'avg_services_touched' => count($serviceSamples) > 0 ? round(array_sum($serviceSamples) / count($serviceSamples), 2) : 0.0,
            'avg_lead_time_days' => count($leadSamples) > 0 ? round(array_sum($leadSamples) / count($leadSamples), 2) : 0.0,
            'avg_coordination_points' => count($coordinationSamples) > 0 ? round(array_sum($coordinationSamples) / count($coordinationSamples), 2) : 0.0,
