...
{
    $dir = sys_get_temp_dir() . '/pdsl-case12';
    if (!is_dir($dir)) {
        mkdir($dir, 0777, true);
    }
...
{
    return storageDir() . '/knowledge-state.json';
}
...
function recordRequestTelemetry(string $uri, int $status, float $elapsedMs, ?array $context): void
{
    ...
    if ($context !== null) {
        ...
        if (isset($telemetry['modes'][$mode])) {
            ...
            $scenario = (string) ($context['scenario'] ?? 'owner_available');
            $modeBucket['by_scenario'][$scenario] = ($modeBucket['by_scenario'][$scenario] ?? 0) + 1;
            appendModeSample($modeBucket, 'mttr_samples', (float) ($context['mttr_min'] ?? 0));
            appendModeSample($modeBucket, 'blocker_samples', (float) ($context['blocker_count'] ?? 0));
            appendModeSample($modeBucket, 'handoff_samples', (float) ($context['handoff_quality'] ?? 0));
...
function telemetrySummary(array $telemetry): array
{
    ...
    foreach (($telemetry['modes'] ?? []) as $mode => $bucket) {
        $mttrSamples = $bucket['mttr_samples'] ?? [];
        $blockerSamples = $bucket['blocker_samples'] ?? [];
        $handoffSamples = $bucket['handoff_samples'] ?? [];
        $modes[$mode] = [
            'successes' => (int) ($bucket['successes'] ?? 0),
            'failures' => (int) ($bucket['failures'] ?? 0),
            'avg_mttr_min' => count($mttrSamples) > 0 ? round(array_sum($mttrSamples) / count($mttrSamples), 2) : 0.0,
            'avg_blocker_count' => count($blockerSamples) > 0 ? round(array_sum($blockerSamples) / count($blockerSamples), 2) : 0.0,
            'avg_handoff_quality' => count($handoffSamples) > 0 ? round(array_sum($handoffSamples) / count($handoffSamples), 2) : 0.0,
