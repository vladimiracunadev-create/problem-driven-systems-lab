...
{
    $dir = sys_get_temp_dir() . '/pdsl-case06';
    if (!is_dir($dir)) {
        mkdir($dir, 0777, true);
    }
...
{
    return storageDir() . '/delivery-state.json';
}
...
function baseEnvironment(string $release, string $configHash, string $secretState): array
{
    return [
...
function initialState(): array
{
    return [
        'environments' => [
            'dev' => baseEnvironment('2026.04.0', 'dev-baseline', 'ready'),
            'staging' => baseEnvironment('2026.04.0', 'staging-baseline', 'ready'),
            'prod' => baseEnvironment('2026.04.0', 'prod-baseline', 'ready'),
        ],
        'history' => [],
    ];
}
...
function recordRequestTelemetry(string $uri, int $status, float $elapsedMs, ?array $context): void
{
    ...
    if ($context !== null) {
        ...
        if (isset($telemetry['modes'][$mode])) {
            ...
            $environment = (string) ($context['environment'] ?? 'staging');

            $stats['by_scenario'][$scenario] = ($stats['by_scenario'][$scenario] ?? 0) + 1;
            $stats['by_environment'][$environment] = ($stats['by_environment'][$environment] ?? 0) + 1;
            $stats['rollbacks'] += !empty($context['rollback_performed']) ? 1 : 0;
            $stats['preflight_blocks'] += !empty($context['preflight_blocked']) ? 1 : 0;
        ...
        $deployment = $context;
        $deployment['status_code'] = $status;
        $deployment['elapsed_ms'] = round($elapsedMs, 2);
        $deployment['timestamp_utc'] = gmdate('c');
        $telemetry['deployments'][] = $deployment;
        if (count($telemetry['deployments']) > 60) {
            $telemetry['deployments'] = array_slice($telemetry['deployments'], -60);
        }
    }
...
function telemetrySummary(array $telemetry): array
{
    ...
    return [
        ...
        'recent_deployments' => array_reverse($telemetry['deployments'] ?? []),
    ];
}
