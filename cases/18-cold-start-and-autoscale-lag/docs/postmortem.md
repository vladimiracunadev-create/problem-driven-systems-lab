...
`/health` es el endpoint por defecto del framework y responde 200 en cuanto el servidor HTTP levanta, **antes** del contexto. Sin `readinessProbe`, Kubernetes trata a un pod como listo apenas el contenedor arranca. El `Service` empezó a enrutarle tráfico a los 18 segundos de distancia de poder atenderlo.
