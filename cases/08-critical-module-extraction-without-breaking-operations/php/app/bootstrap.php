...
{
    $dir = sys_get_temp_dir() . '/pdsl-case08';
    if (!is_dir($dir)) {
        mkdir($dir, 0777, true);
    }
...
{
    return storageDir() . '/extraction-state.json';
}
...
function recordRequestTelemetry(string $uri, int $status, float $elapsedMs, ?array $context): void
{
    ...
    if ($context !== null) {
        $mode = (string) ($context['mode'] ?? 'bigbang');
        if (isset($telemetry['modes'][$mode])) {
            $modeBucket = &$telemetry['modes'][$mode];
            $scenario = (string) ($context['scenario'] ?? 'stable');
            ...
            appendModeSample($modeBucket, 'compatibility_hit_samples', (float) ($context['compatibility_hits'] ?? 0));
            appendModeSample($modeBucket, 'consumer_progress_samples', (float) ($context['consumer_progress'] ?? 0));
...
function telemetrySummary(array $telemetry): array
{
    ...
    foreach (($telemetry['modes'] ?? []) as $mode => $bucket) {
        ...
        $compatibilitySamples = $bucket['compatibility_hit_samples'] ?? [];
        $progressSamples = $bucket['consumer_progress_samples'] ?? [];
        $modes[$mode] = [
            ...
            'avg_compatibility_hits' => count($compatibilitySamples) > 0 ? round(array_sum($compatibilitySamples) / count($compatibilitySamples), 2) : 0.0,
            'avg_consumer_progress' => count($progressSamples) > 0 ? round(array_sum($progressSamples) / count($progressSamples), 2) : 0.0,
