...
- El laboratorio modela **16 problemas reales de ingeniería**, utilizando fallos de alta fidelidad inyectados (I/O, Memoria, Excepciones) en lugar de simulaciones abstractas.
...
El laboratorio no es solo una "API JSON ciega". Los 16 casos en PHP ahora interceptan solicitudes HTTP de navegadores (mediante cabeceras `Accept`) y devuelven **Dashboards Interactivos**. Esto permite a reclutadores, líderes y desarrolladores *ver* cómo se bloquea una base de datos, cómo aumentan las latencias, y probar escenarios en vivo usando estéticas modernas sin afectar el núcleo programático.
...
| --- | --- | --- |
| Recruiter / hiring manager | [RECRUITER.md](RECRUITER.md) → [docs/executive-summary.md](docs/executive-summary.md) | El repo deja evidencia real y los 16 casos caben en una pagina ejecutiva |
...
Si quieres una sola puerta de entrada local con los 16 casos PHP disponibles, levanta `docker compose -f compose.root.yml up -d --build` y abre `http://localhost:8080`.
...
![Cobertura real de los 16 casos en los 7 stacks](docs/assets/stack-matrix.svg)

## 🧬 Perfiles de lenguaje y mantenimiento por versión

Los 16 casos no resuelven problemas con código genérico: los resuelven con **la primitiva idiomática de cada runtime**. Eso es lo que hace comparables a los siete stacks, y también lo que caduca cuando un lenguaje evoluciona.

**[📁 docs/languages/](docs/languages/README.md) — un perfil por lenguaje**, cada uno con seis secciones: identidad y para qué sirve, modelo de ejecución, primitivas usadas en los 16 casos con enlace al código, qué mide el laboratorio y cómo reproducirlo, límites y problemas sin solución, y ciclo de versiones.
...
Cada lenguaje tiene su propio compose en la raíz del repositorio. Un comando levanta los 16 casos de ese lenguaje. Los stacks son independientes y pueden correr en paralelo sin colisión de puertos.
...
**Siete hubs operativos (uno por lenguaje):** PHP, Python, Node, Java, .NET, Go y Rust sirven los **16 casos cada uno**. Un solo puerto por hub vía routing por path (`/01/health`...`/16/health`). Los servicios de soporte (DB, Prometheus, Grafana) tienen los suyos propios porque son servicios distintos del lenguaje.

> 🧱 **Los siete hubs siguen el mismo patrón arquitectónico:** un contenedor por lenguaje (`pdsl-php-lab`, `pdsl-python-lab`, `pdsl-node-lab`, `pdsl-java-lab`, `pdsl-dotnet-lab`, `pdsl-go-lab`, `pdsl-rust-lab`) ejecuta sus 16 casos como subprocesos internos en puertos no expuestos. PHP suma ~7 contenedores extras solo porque los **servicios reales** que el caso 01 estudia (PostgreSQL, worker, Prometheus, Grafana) son contenedores aparte por necesidad técnica — no son procesos del lenguaje. Detalles, trade-offs y comparación per-case en [`docs/docker-strategy.md`](docs/docker-strategy.md#-modelo-de-containerización-simétrico-para-los-stacks-operativos).

```bash
# PHP: portal + dispatcher (16 casos internos en un contenedor) + DB + Prometheus + Grafana
docker compose -f compose.root.yml up -d --build

# Python: dispatcher (16 casos internos en un contenedor)
docker compose -f compose.python.yml up -d --build

# Node.js: dispatcher (16 casos internos en un contenedor)
docker compose -f compose.nodejs.yml up -d --build

# Java: dispatcher (16 casos internos en un contenedor)
docker compose -f compose.java.yml up -d --build

# .NET 8: dispatcher (16 casos internos en un contenedor)
docker compose -f compose.dotnet.yml up -d --build

# Go 1.23: dispatcher (16 casos internos en un contenedor)
docker compose -f compose.go.yml up -d --build

# Rust 1.83: dispatcher (16 casos internos en un contenedor)
...
Con esto, los 112 endpoints operativos (16 casos × 7 stacks) viven detras de **7 puertos**: `8100`, `8200`, `8300`, `8400`, `8500`, `8600`, `8700`. El portal (`8080`) y la observabilidad (`9091` Prometheus, `3001` Grafana) suman 3 mas. **10 puertos cubren el laboratorio entero.**
...
| [docs/executive-summary.md](docs/executive-summary.md) | 📋 Resumen ejecutivo: los 16 casos en una pagina (problema · valor · evidencia) |
...
El sistema se organiza como una capa editorial en raiz, un portal de evaluacion con entrada completa PHP o modo liviano, una biblioteca de 16 casos problem-driven y **siete stacks operativos** detras de hubs simetricos por lenguaje (PHP/Python/Node/Java/.NET/Go/Rust los 16 casos cada uno). La arquitectura completa esta documentada en [ARCHITECTURE.md](ARCHITECTURE.md) y [docs/architecture.md](docs/architecture.md).
...
- Paridad multi-stack universal a nivel de feature: los siete stacks (PHP, Python, Node, Java, .NET, Go, Rust) cubren los 16 casos cada uno con primitivas idiomáticas distintas — no es paridad sintáctica, es paridad funcional con criterio por runtime.
