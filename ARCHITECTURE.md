...
**Estado a la fecha:** 7 stacks operativos × 13 casos = **91 endpoints** detrás de 7 hubs simétricos.
...
Cada lenguaje operativo tiene su compose raíz — un comando levanta los 13 casos de ese lenguaje:
...
| `compose.python.yml` | Python 3.12 | `:8200` | 13 casos en un solo contenedor dispatcher, stdlib pura |
| `compose.nodejs.yml` | Node.js 20 | `:8300` | 13 casos en un solo contenedor, stdlib pura |
| `compose.java.yml` | Java 21 | `:8400` | 13 casos en un solo contenedor, JDK built-in (`HttpServer`, `HttpClient`) |
| `compose.dotnet.yml` | .NET 8 | `:8500` | 13 casos en un solo contenedor, BCL built-in (`HttpListener`, `System.Text.Json`) |
| `compose.go.yml` | Go 1.23 | `:8600` | 13 casos en un solo contenedor, stdlib (`net/http`, `encoding/json`, `httputil.ReverseProxy`) |
| `compose.rust.yml` | Rust 1.83 | `:8700` | 13 casos en un solo contenedor; `std` **no** trae HTTP ni JSON — capa propia sobre `TcpListener` |
...
Los 13 casos operativos en PHP detectan `Accept: text/html` y devuelven un **dashboard nativo construido en Vanilla JS/CSS** (`ui.php`), sin frameworks. Esto permite que recruiters y líderes *vean* el problema sin tener que hacer `curl`.
...
Los siete hubs siguen el **mismo patrón**: un contenedor por lenguaje ejecuta sus 13 casos como subprocesos internos en puertos no expuestos.
...
graph LR
    ...
    subgraph C03_12[03-12 patrones idiomáticos]
        OTHER[13 casos x 7 stacks operativos]
    end
```
...
**Decisión:** un compose raíz por lenguaje (`compose.<lang>.yml`) que levanta los 13 casos de ese lenguaje en un solo contenedor con subprocesos internos.
...
**Por qué:** el portal no es un índice de docs muerto — es una **demo verificable en vivo**. Un recruiter abre `localhost:8080`, ve verde en los 13 casos del stack que eligió, y sabe que **el repo está vivo en este momento**.
...
**Decisión:** los 13 casos de un stack viven en el mismo contenedor (subprocesos del dispatcher).
...
| `compose.python.yml` | Python: dispatcher único con 13 casos internos (`:8200`) |
| `compose.nodejs.yml` | Node: dispatcher único con 13 casos internos (`:8300`) |
| `compose.java.yml` | Java 21: dispatcher único con 13 casos internos (`:8400`) |
| `compose.dotnet.yml` | .NET 8: dispatcher único con 13 casos internos (`:8500`) |
| `compose.go.yml` | Go 1.23: dispatcher único con 13 casos internos (`:8600`) |
| `compose.rust.yml` | Rust 1.83: dispatcher único con 13 casos internos (`:8700`) |
